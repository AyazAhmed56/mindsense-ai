...
export default function Dashboard() {
  const { currentState, responseMode, currentSpeed, currentError, currentPause } =
    useCognitiveState();
  ...
  const getDashboardContent = () => {
    if (currentState === "Focused") {
      return {
        title: "Detailed Mode",
        description: "You seem ready for deeper work and advanced explanations.",
        suggestions: [
          "Show detailed answers",
          "Enable advanced mode",
          "Productivity looks good",
        ],
        cardClass: "bg-green-100 border-green-300 text-green-700",
      };
    }
    ...
    if (currentState === "Confused") {
      return {
        title: "Guided Mode",
        description: "You may benefit from simpler, step-by-step help.",
        suggestions: [
          "Show step-by-step mode",
          "Suggest examples",
          "Break tasks into smaller parts",
        ],
        cardClass: "bg-yellow-100 border-yellow-300 text-yellow-700",
      };
    }

    return {
      title: "Low Load Mode",
      description: "A short break or simpler content may help right now.",
      suggestions: [
        "Show short answers",
        "Suggest a short break",
        "Reduce cognitive load UI",
      ],
      cardClass: "bg-red-100 border-red-300 text-red-700",
    };
  };

  const config = getDashboardContent();

  return (
    <div className="min-h-screen bg-gray-100 p-8">
      <div className={`border rounded-2xl p-8 ${config.cardClass}`}>
        ...
        <div className="grid md:grid-cols-3 gap-4 mb-6">
          ...
          <div className="bg-white/70 rounded-xl p-4 border">
            ...
            <p className="text-2xl font-bold">{currentPause}</p>
          </div>
        </div>
